<template>
    <ul role="list" class="rounded-lg shadow-xl border border-slate-400 dark:border-slate-700 mx-6">
        <li v-for="command, index in props.data" :key="index" class="grid py-4 px-2 grid-cols-12 max-w-2xl
            ...
            <div class="col-span-6 sm:col-span-2 text-left font-extrabold">
                {{ command.title }}
            </div>
            <div :class="cli ? 'sm:col-span-8' : 'sm:col-span-8'"
                class="shrink-0 col-span-12 order-3 pt-2 sm:pt-0 sm:order-2 text-right flex">
                <div class="shrink-0">
            ...
            <div :class='cli && "justify-end w-full"' class="sm:col-span-2 col-span-6 sm:order-3 text-right flex gap-x-1 sm:m-auto ml-auto
                ...
                <div :title="$t('copy')">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                        @click="copy(command.url, command.data, command.curlEnd)" stroke="currentColor" stroke-width="2">
